...
from ..deformable_detr.deformable_detr import PostProcess
...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
                 ...
                 backbone_align=False, level_align=False, space_align=False, channel_align=False, instance_align=False):
        ...
        self.uda = backbone_align or level_align or space_align or channel_align or instance_align
        ...
        #if level_align:
        #    self.level_D = MLP(hidden_dim, hidden_dim, 1, 3)
        #    for layer in self.level_D.layers:
        #        nn.init.xavier_uniform_(layer.weight, gain=1)
        #        nn.init.constant_(layer.bias, 0)
        if space_align:
    ...
    def forward(self, samples: NestedTensor):
        ...
        # TODO: placed here for hooks, remove afterwards
        ...
        if self.training and self.uda:
            ...
            if self.backbone_align:
                da_output['backbone'] = torch.cat([self.backbone_D(self.grl(src.flatten(2).transpose(1, 2))) for src in srcs], dim=1)
            #if self.level_align:
            #    da_output['level_query'] = self.level_D(da_output['level_query'])
            if self.space_align:
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # TODO: Fix, it shouldn't be needed if using the DetOnlyDataset wrapper
        if self.num_classes <= 2:
            for i in range(len(targets)):
                targets[i]['labels'] = torch.ones_like(targets[i]['labels']).to(targets[i]['labels'].device)
