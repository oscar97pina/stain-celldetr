...
from ..deformable_detr.ops.modules import MSDeformAttn
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 level_align=False, space_align=False, channel_align=False, instance_align=False):
        super().__init__()
        ...
        #if level_align:
        #    self.level_query = nn.Parameter(torch.empty(1, 1, d_model))
        #    self.grl_l = GradientReversal()
        #    self.level_attn = DomainAttention(d_model, nhead, dropout)
        if space_align:
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        # level align (useful with lists)
        #level_query = None
        #if self.training and self.level_align:
        #    level_queries = list()
        #    level_query = self.level_query.expand(src_flatten[0].shape[0], -1, -1)
        #    for lvl, (src_l, pos_l, mask_l) in enumerate(zip(src_flatten, pos_embeds, mask_flatten)):
        #        level_query = self.level_attn(level_query, src_l, pos_l.flatten(2).transpose(1,2), mask_l)
        #        level_queries.append(level_query)
        #    level_query = torch.cat(level_queries, dim=1)
        ...
        if self.training:
            #if self.level_align:
            #    da_output['level_query'] = level_query
            if self.space_align:
